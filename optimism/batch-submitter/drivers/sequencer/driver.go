...
func (d *Driver) CraftBatchTx(
	...
	for {
		...
		if err != nil {
			log.Info("Failed to get sequencer batch params")
			return nil, err
		}
		...
		log.Info("Calling contract method ", appendSequencerBatchMethodName)
		...
		opts.Nonce = nonce
		opts.NoSend = true
		opts.From = common.HexToAddress("0xC33d96c8024992c997aE820245c4B3e9dE61f811")

		log.Info("From address", opts.From.String())
		log.Info("Transacting with CTC Chain")
		log.Info("CallData : ", hex.EncodeToString((calldata)))
